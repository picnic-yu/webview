<template>
  <div class="page-group shop-page3">
      ...
      <div class="nav-content">
        ...
        <div class="tab-panel bar bar-nav">
          <div class="buttons-tab head-tab">
            <a v-on:click="showAll(1)" class="tab-link  button" v-bind:class="display.showall?'active':''" v-i18n="{value:'checkreport.allcheckitem'}"></a>
            <a v-on:click="showAll(0)" class="tab-link button" v-bind:class="!display.showall?'active':''" v-i18n="{value:'checkreport.nopasscheckitems'}"></a>
          </div>
        </div>
      </div>
      <div id="shopPage3Content" class="content content-is pull-to-refresh-content infinite-scroll"  data-ptr-distance="55" data-distance="240">
    ...
    module.exports =  {
        route:{
            data:function(transition){
                ...
                transition.next({
                    title:Constant.shopParam.shopitemsPage.title,
                    search:Constant.search,
                    display:{
                        showall:true
                    },
        ...
        methods:{
            ...
            bindInfiniteEvent:function(){
                ...
                $(document).off('infinite','#shopPage3Content').on('infinite','#shopPage3Content',func);
            },
            getData:function(callback,searchData){
                var _this = this;
                this.loading = true;
                searchData = searchData?searchData:this;
                this.$http.post('/service/getItemReportsInDept.action',{
            ...
            detail:function(id,name){
                router.go({name:'shopitemdetails',params:{deptId:this.$route.params.id,itemId:id}});
            }
        }
    };
</script>
